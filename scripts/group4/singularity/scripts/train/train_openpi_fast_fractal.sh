...
CUDA_VISIBLE_DEVICES=4,5 uv run "$OPENPI_DIR_SIF/scripts/compute_norm_stats.py" --config-name pi0_fast_fractal_low_mem_finetune || { echo "Failed to compute normalization statistics"; exit 1; }

# Train the model
# Use GPU 0 and limit memory usage to 90% to avoid OOM errors
CUDA_VISIBLE_DEVICES=4,5 XLA_PYTHON_CLIENT_MEM_FRACTION=0.9 uv run "$OPENPI_DIR_SIF/scripts/train.py" pi0_fast_fractal_low_mem_finetune --exp-name=my_experiment --overwrite || { echo "Failed to train the model"; exit 1; }