...
if __name__ == "__main__":
    ...
    for ckpt_path in ckpt_paths:
        for env_name in env_names:
            ...
            success_rate = np.array(success_arr).mean()
            print(f"Success rate for {env_name}: {success_rate}")
            results.append(("Scene1", env_name, ckpt_path, success_rate))
    ...
    for ckpt_path in ckpt_paths:
        for env_name in env_names:
            ...
            success_rate = np.array(success_arr).mean()
            print(f"Success rate for {env_name}: {success_rate}")
            results.append(("Scene2", env_name, ckpt_path, success_rate))
    print("\n=== Summary ===")
    for scene, env_name, ckpt_path, success_rate in results:
        print(f"{scene} | {env_name} | ckpt={ckpt_path} | success_rate={success_rate:.3f}")
